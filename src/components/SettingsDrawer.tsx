  ...
  return (
    <Sheet open={open} onOpenChange={(v) => { setOpen(v); if (v) { setDraft(apiUrl); setTestResult(null); } }}>
      <SheetTrigger asChild>
        <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="Paramètres">
          <Settings className="h-4 w-4" />
        </Button>
      </SheetTrigger>
      <SheetContent className="bg-card border-border">
        <SheetHeader>
          <SheetTitle className="text-foreground">Paramètres</SheetTitle>
        </SheetHeader>
        <div className="mt-6 space-y-5">
          <div className="space-y-2">
            <label className="text-sm font-medium text-foreground">URL du serveur API</label>
            <input
              value={draft}
          ...
          {testResult && (
            <div className={`flex items-center gap-2 text-sm font-medium ${testResult === "ok" ? "text-green-400" : "text-destructive"}`}>
              ...
              {testResult === "ok" ? "✅ Serveur en ligne" : "❌ Serveur hors ligne"}
            </div>
          )}
